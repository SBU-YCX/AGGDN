...
class NGDCSDE(nn.Module):
    ...
    def forward(self, data_batch, delta_t=None, numSample=None, return_h=True):
        ...
        for n in t_iter:                                               
            ...
            if not return_h:  # training
                Z = self.SDE.solve_sde(self.sde_method, Z_post.reshape(-1, self.dimState), time_points)[-1].reshape(Z.size(0), Z.size(1), Z.size(2), Z.size(3))
                kld_t = (Z_post - Z_prior)**2 / torch.max(
                    Z_post**2,
                    torch.Tensor([1e-2]).cuda())
                kld = kld + delta_t * kld_t.sum((-3, -2, -1)).mean()
            else:
                Z = self.SDE.solve_sde(self.sde_method, Z_prior.reshape(-1, self.dimState), time_points)[-1].reshape(Z.size(0), Z.size(1), Z.size(2), Z.size(3))
            #Zs.append(Z)
    ...
    def get_loss(self, data_batch):
        ...
        return 0.5 * mse_v + 0.5 * mse_ode + self.beta * kld, X_pred
