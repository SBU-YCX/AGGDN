...
import numpy as np
from models.vsdn.Modules import GRUObservationCell, init_weights
from models.vsdn.losses import Gaussian_NLL


class ODE_RNN_MODEL(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.gru_c = nn.Sequential(
            nn.Linear(rnn_channels, decoder_hidden_channels, bias=False),
            nn.ReLU(),
            nn.Dropout(p=dropout_rate),
            nn.Linear(decoder_hidden_channels, rnn_channels, bias=False),
            nn.Tanh(),
        )
    ...
    def ode_step(self, h, p, delta_t, current_time):
        ...
        if self.solver == "euler":
            h = h + delta_t * 0.1 * self.gru_c(h)
            p = self.p_model(h)

        elif self.solver == "midpoint":
            k = h + delta_t / 2 * self.gru_c(h)
            h = h + delta_t * 0.1 * self.gru_c(k)
            p = self.p_model(h)
...
class ODE_RNN_MODEL_Backward(torch.nn.Module):
    """
        # GRU_ODE in backward direction.
    """
    def __init__(self,
                 input_channels,
                 rnn_channels,
        ...
        super(ODE_RNN_MODEL_Backward, self).__init__()
        self.p_model = nn.Sequential(
            nn.Linear(rnn_channels, decoder_hidden_channels, bias=bias),
            nn.ReLU(),
            nn.Dropout(p=dropout_rate),
    ...
    def forward(self,
        ...
        p = self.p_model(h)
        # Initiate the recurrent operations.
        current_time, loss = delta_t * X.size(
        )[0], 0  # Pre-jump loss and Post-jump loss (KL between p_updated and the actual sample)
        if return_path:
            path_t, path_p, path_h, path_hpos = [current_time], [p], [h], [h]
        for i, Xt in enumerate(torch.flip(X, dims=[0])):
            ## Propagation of the ODE until next observation
            Mt = M[i].mean(-1, keepdim=True)
            h_pre, p_pre, current_time = self.ode_step(h, p, delta_t,
                                                       current_time)
            ...
            if return_path:
                ...
                path_hpos.append(h)
        if return_path:
            path_t, path_p, path_h, path_hpos = np.array(path_t), torch.stack(
                path_p), torch.stack(path_h), torch.stack(path_hpos)
            path_t, path_p, path_h, path_hpos = np.flip(path_t, axis=0), torch.flip(path_p, dims=[0]), \
                                                torch.flip(path_h, dims=[0]), torch.flip(path_hpos, dims=[0])
            return h, loss, path_t, path_p, path_h, path_hpos
        else:
            return h, loss
